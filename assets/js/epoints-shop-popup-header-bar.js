...
const mainBlock = document.getElementById("tbay-header");
...
console.log(urlParams.get('maxLimit'));

['.group-buttons','#sticky-menu-bar','#tbay-footer','.tbay-woocommerce-breadcrumb','#product-related','#sidebar-shop'].forEach((el,i)=>$('body.chrome-less-reward '+el).remove());


let generateHeaderBar = () => {
  console.log("header bar generated");
  return (mainBlock.innerHTML += `<div class="gren-border">
 <span class="epoints-price-text epoints-price-price epoints-price-text-small font-styleRegular" automation-id="epoints-price-price">You have `+currencyObject.currency+` <span id="cl-hb-price"> ${(maxepointLimit/((currencyObject.conversionRate || 1) * 200)).toFixed(2)} </span></span><span class="ep-dots"> • </span>
 <span class="epoints-points-in-nums font-weight-bold"> <img src="https://qa-shop.eachperson.com/wp-content/themes/besa-child/assets/img/elogo-rebrand-small.png" class="epoints-logo-rebranded" width="14px"> <span  id="cl-epoints-price">${maxepointLimit}</span></span>
 <span> to use due to the reason selected.</span>
 </div>
</div><div class="red-border"><span id="warning-limit"></span></div>`);
};
...
currentLimit = Object.keys(data)
                    .map((r) => data[r].price * data[r].quantity * 200 )
                    .reduce((a, b) => a + b, 0);
	warningText = ``;
if (currentLimit > maxepointLimit)
warningText = ` Your cart now exceeds the maximum reward criteria `;
function updateEpointsheader() {
    	const epointsText = document.getElementById('cl-epoints-price');
        const clepPrice = document.getElementById('cl-hb-price');
		epointsText.innerText = (maxepointLimit) - currentLimit
		clepPrice.innerText = (((maxepointLimit) - currentLimit)  / 200)
    }
			  ...
			  ;}
                ...
                Object.keys(data).forEach((id, i) => {
                    ...
                    finalPayLoad["amountInCurrency"] = data[id].price * rate;
			  		finalPayLoad["productDetail"] = data[id].redemptionItemInfo;
			        finalPayLoad["giftCard"] = data[id].detectgiftCard;
...
let generatequantityBlock = () => {
  return (mainBlock.innerHTML += `<div class="cl-icon-quantity">
...
if (ssInitialValue != null && document.body.classList.contains('single-product') ) {
  generatequantityBlock();
}
